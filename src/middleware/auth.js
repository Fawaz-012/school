...
export const auth = async (req, res, next) => {
  try {
    ...
    if (!authHeader) {
      return res.status(401).json({ error: "Unathorized, no authHeader" });
    }
    ...
    jwt.verify(token, config.access, (error, user) => {
      if (error) {
        return res
          .status(403)
          .json({ error: "This session has expired. Kindly re-login" });
      }
...
//     if (user.role !== "admin")     
//         return res.status(403).json({messsage : `You do not have access to this endpoint`});
//    next(); 
//     } catch (error) {
